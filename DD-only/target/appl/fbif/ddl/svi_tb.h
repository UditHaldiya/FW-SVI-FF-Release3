...
{                                                                      \
    tid = id;                                                          \
    tm_id = mid;                                                       \
    get_acknowledgement("|en|Id:%[d]{tid}, %[d]{tm_id}", dummy, dummy, 0); \
}                                                                      \
...
#define TEXT_STROKE_VALVE                                                               \
    ...
    "Click Next to proceed or Abort now" \
...
#define COMMON_LOCAL_VARS												\
	...
	char     str_buf[256];												\
	long     maxlen;													\
	int tries /*This pre-definition must be ended by semo-comma*/		\
    TEST_VARS                                                          \
...
#define METHOD_FINISHED_MACRO \
    get_acknowledgement("|en|Finished", dummy, dummy, 0); \
    return;

#define METHOD_ABORT_MACRO \
    get_acknowledgement("|en|Aborted", dummy, dummy, 0); \
    return;

#define METHOD_EXIT_MACRO \
    get_acknowledgement("|en|The method exits", dummy, dummy, 0); \
    return;

#define IF_ERROR_ABORT_MACRO_INFO(prompt)   \
    if ( BLTIN_SUCCESS !=status) { \
        resp_code = status;                                              \
        get_acknowledgement("|en|" prompt "(%[d]{status})\n", dummy, dummy, 0); \
...
/* This macro relies on CHECK_PROCESS being a dynamic variable,
so there is no need to read it.

Even if we want to run a process that doesn't use the diagnostic buffer,
we still need to cancel the previous process to stuff in the new one.
So, check on dataset is performed unconditionally.
Alas, a non-process (e.g. "manual find stops" will also discard dataset.
Anyone cares?
*/
#if 0
#define CHECK_DEVICE_IN_PROCESS  \
    status = get_unsigned_value(ITEM_ID(PARAM.CHECK_PROCESS), MEMBER_ID(PROCESS_ID), &us8_buf);     \
    if ( us8_buf != process_none ){ \
        status = get_unsigned_value(ITEM_ID(PARAM.CHECK_PROCESS), MEMBER_ID(PERCENT_COMPLETE), &us8_buf);     \
        if(us8_buf > 150) { \
            get_acknowledgement("|en|The device has a data set collected.\n" \
                            "Click Next to discard it, or Abort the method now.", dummy, dummy, 0); \
            status = send_value(ITEM_ID(OFFLINE_DIAGNOSTIC), 0, DIAG_CANCEL_TEST);   \
            delayfor(2, "|en|Discarding the data set...", dummy, dummy, 0);  \
        } \
        else { \
            get_acknowledgement("|en|The device is running a process.\n"                                  \
                            "Please wait for the process to end.\n\n" \
                            "The method will now terminate.", dummy, dummy, 0); \
            METHOD_ABORT_MACRO \
        } \
    } \
}
...
#define CHECK_DEVICE_IN_PROCESS  \
    ...
    if ( us8_buf != process_none ){\
        ...
        if(us8_buf > 150) {\
            get_acknowledgement("|en|The device has a data set collected.\n"\
                            "Next to discard it, or Abort the method now", dummy, dummy, 0);\
            put_unsigned_value(ITEM_ID(OFFLINE_DIAGNOSTIC), 0, DIAG_CANCEL_TEST);\
            status = send_value(ITEM_ID(OFFLINE_DIAGNOSTIC), 0);\
            delayfor(2, "|en|Discarding the data set...", dummy, dummy, 0);\
        }\
        else {\
            get_acknowledgement("|en|The device is running a process.\n"\
...
#define CHECK_DEVICE_IN_OOS                                                                     \
        READ_PARAM(ITEM_ID(PARAM.MODE_BLK), MEMBER_ID(ACTUAL), "MODE_BLK");                        \
        status = get_unsigned_value(ITEM_ID(PARAM.MODE_BLK), MEMBER_ID(ACTUAL), &us8_buf);      \
                                                                                                \
        if ( us8_buf != _OUT_OF_SERVICE ) { /* TB spec. V37 */                                          \
            get_acknowledgement("|en|MODE_BLK(Actual) is not in OOS mode.\n"                    \
                                "Please set to the OOS mode.\n",dummy,dummy,0);                 \
            METHOD_ABORT_MACRO                                                                  \
        }
...
#define SEND_DIAG_COMMAND(ivalue)							\
	...
	if (ivalue != 0) {												\
		if (mbs[0] >= 150) {										\
			get_acknowledgement("|en|WARNING: This procedure will move the valve.\n"		\
								"|en|The device has a data set collected.\n"\
								"Click Next to discard it, or Abort the method now"	\
								, dummy, dummy, 0);		\
		}else /*if (mbs[0] < 150)*/ {										\
			...
			send_all_values();													\
			get_acknowledgement("|en|%{1} \n", ids, mbs, 1);			\
			abortme = 0;															\
		}																\
		while (abortme != 0) {											\
				get_acknowledgement("|en| %L{1} = %{1}", ids, mbs, 1);		\
				/*Cancel process*/												\
				SEND_PARAM(ids[1], mbs[1], 99);									\
				status = put_unsigned_value(ids[1], mbs[1], DIAG_CANCEL_TEST);				\
				status = send_value(ids[1], mbs[1]);							\
				send_all_values();												\
				get_acknowledgement("|en|%L{1} = %{1}", ids, mbs, 1);				\
...
#define HLP_FUNCTION                                                                    \
  "|en|Switch configuration selection:"        \
...
#define AUTOTUNE_FAILED_STABILIZE 30
...
#define LBL_NEAR_CLOSE_ALERT             "|en|Near Closed Alert"
...
#define HLP_CAL_LOCATION                                                          \
  "|en| The location of last positioner calibration. Describes the physical" \
  " location at which the calibration was performed."   \

#define HLP_CAL_TYPE                                                              \
  "|en| Last Travel Calibration Result."                                                   \
  "\nSet when the calibration procedure is finished."
...
#define HLP_NEAR_CLOSED                                                           \
  "|en|Time in hours the valve spends below the configured near-closed position "      \
...
#define HLP_PERCENT_COMPLETE                "|en|Percent complete > 100 % indicates that collected data is left in the device. Either discard, or upload via DTM"                                \
...
#define HLP_045_FINAL_VALUE                                                             \
  "|en|Requested valve position and status written by AO FB"          

#define HLP_046_STATUS                                                                  \
  "|en|Status written by the AO FB"                         

#define HLP_048_FINAL_VALUE_RANGE                                                       \
  "|en|Range of the FINAL_VALUE (setpoint) and the FINAL_VALUE_RANGE"              \
  "(actual position)."                                                                  \
  "\not changeable, so the scale parameter of the AO"            \
  " FB has to be limited to provide these values only"                     
...
#define HLP_064_CUTOFF_POINT_HI                                                         \
  "|en| Cut-Off point HI. Valve will be fully open after this point"                 \
...
#define HLP_067_CUTOFF_POINT_LO                                                         \
  "|en|Cut-Off point LO. Valve will be fully closed after this point"               \
...
#define HLP_071_ACTIVATE_CONTROL_SET                                                    \
  ...
  "\n"                                                                                  \
  "\n Copy Backup Control Set To Custom"                       \
  "\n Copy Active Control Set To Custom"                              \
  "\n Do Nothing"                                                                 \
  "\n"                                                                                  \
...
#define HLP_100_TRAVEL_CALIBRATION                                                      \
  "|en|Physical location of last positioner calibration"       \

#define HLP_108_TRAVEL                          "|en|"                                  \

#define HLP_109_RANGE                                                                   \
  "|en| A numeric value describing the Range of travel. Used to"               \
...
#define HLP_113_VALUE                                                                   \
  "|en|The value converted to TRAVEL.UNITS_INDEX" \
...
#define HLP_270_ALERT_POINT                                                             \
  "|en|Value above which ALERT is triggered. Note: this time must"       \
...
#define HLP_278_FSTATE_VALUE                                                            \
  "|en|The preset fallback analog setpoint to use when fault occurs. This"        \
...
#define HLP_294_PRESSURE_RANGE                                                          \
  "|en|The range of supply pressure"                                              \
...
#define HLP_320_ACTUATOR_A_PRESSURE                                                     \
  "|en|Actuator control pressure with status"                                        \
...
#define HLP_326_ATMOSPHERIC_PRESSURE                                                    \
  "|en|Atmospheric pressure with status"                                    \
...
#define HLP_332_PRESSURE_CALIBRATION                                                    \
  "|en|Information about the last positioner pressure calibration"               \
...
#define HLP_341_TEMPERATURE                                                             \
  "|en|Temperature of the electronic unit"                          \
...
#define HLP_376_AUTOTUNE                                                                \
  ...
  " Tune Flags to the process. The fourth parameter is ignored on" \
  " writing. Reading will return same as written values of the first 3 parameters" \
  " plus the fourth parameter - the autotune process" \
  " completion code." \
...
#define HLP_CUSTOM_CONFIGURATION                                                  \
  "|en|Selection of the value displayed"       \
  " on the local LCD display. " \
  "\n 1. TB.SETPOINT - The result of FINAL_VALUE(_D,_DINT) based on SETPOINT_SOURCE, and XD_FSTATE action" \
  "\n 2. TB.FINAL_POSITION_VALUE"      \
  "\n 3. PID1.SP"                                                                       \
  "\n 4. PID1.PV"                                                                       \
  "\n 5. PID1.OUT"                                                                      \
  "\n 6. PID2.SP"                                                                       \
...
#define HLP_445_ACTUAL_POSITION                                                         \
  "|en|Position reported to the controlling FB BKCal_Out per READBACK_SELECT" \
...
#define HLP_561_PRS_CONTROL_HI                                                          \
  "|en|May be the same as Maximum Supply Pressure for the actuator in control"         \


#define HLP_562_PRS_CONTROL_LO                                                          \
  "|en|May be the same as Minimum Supply Pressure for the actuator in control"         \
...
#define LBL_445_ACTUAL_POSITION                 "|en|Readback Position"
...
#define TB_METHOD_LIST  \
	...
	clear_travel_trend, \
	clear_cycle_counter,    \
	clear_position_error_trend,    \
	clear_position_histogram,    \
	clear_position_error_histogram,    \
	clear_working_times,    \
...
#define ENUM_ACTIVATE_CONTROL_SET                                                     \
  ...
  { 11, "|en|11 :: Copy active control set to Custom control set" } \
...
#define ENUM_COMPLETION \
  ...
  {AUTOTUNE_FAILED_STABILIZE,                   "|en|30 - Failed to stabilize the valve"        } \
