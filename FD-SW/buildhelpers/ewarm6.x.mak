ifeq (yin,yang)
 *************** IAR EWARM 6.x setup ******************
    Crutch to remove!!! See bug 29077!

    Only changed the linker command!!! Temporarily!
endif
...
export SYSINCDIR:=$(TOOLDIR)$(SLASH)ARM$(SLASH)inc $(TOOLDIR)$(SLASH)ARM$(SLASH)inc\c
# ------------------ Compiler --------------------
...
export DIAG_SUPPRESS=Pe236 Pe1374 Pe1375 Pa091 Pa039
...
define C_Command
    ...
    $(Hide)echo $(subst BASENAME,$(REALNAME),BASENAME.o BASENAME.d:) \>$(REALNAME).tmp
    $(Hide)$(FIXDEP) $(REALNAME).d_>>$(REALNAME).tmp
    $(Hide)echo $< $(LOCAL_CONF) $(local_topdep)>>$(REALNAME).tmp
    $(Hide)$(MN_MV) $(REALNAME).tmp $(REALNAME).d
endef
...
#ffplib= $(basename $(BTARGET)).a
ffplib=C:\TFSSC\Projects\FF\FD-SW\cmdbuild\Dbg\project_FFP\FFP.a
#ffplib=

# This works:  $(LD) cmdbuild\Dbg\project_FFP\mncbdefs.o $(LDFLAGS_PROJ) $(LDFLAGS) $(basename $(BTARGET)).a $(MAINLIB) -o $(basename $@).out.tmp
# This doesn't:    $(LD) $(subst $(PROJDIR)\,,$(shell $(MN_CAT) $(LINKFILES))) $(LDFLAGS_PROJ) $(LDFLAGS) $(MAINLIB) -o $(basename $@).out.tmp
LDARG=cmdbuild\$(DIR)\project_FFP\mncbdefs.o
#-- LDARG=$(subst $(PROJDIR)\,,$(shell $(MN_CAT) $(LINKFILES)))
#LDARG=-f $(LINKFILES)
#-- LDARG=--redirect _Printf=_PrintfFull --redirect _Scanf=_ScanfFull -f $(LINKFILES)

# Link command (we are not shy to include linker post-processing commands in it)
define LinkCmd
    $(LD) $(LDFLAGS_PROJ) $(LDFLAGS) $(ffplib) $(MAINLIB) $(LDARG) -o $(basename $@).out.tmp
