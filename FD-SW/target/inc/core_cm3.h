 ...
 *   - Error 750: \n
 *     #define CM3_CORE_H_ \n
 *     Info 750: local macro 'CM3_CORE_H_' (line 43, file./cm3_core.h) not referenced
 * .
 *   - Error 528: \n
...
//Systick moved to stm_systick.h #define SysTick_BASE        (SCS_BASE +  0x0010)                      /*!< SysTick Base Address              */
#define NVIC_BASE           (SCS_BASE +  0x0100)                      /*!< NVIC Base Address                 */
//#define SCB_BASE            (SCS_BASE +  0x0D00)                      /*!< System Control Block Base Address */

#define InterruptType       ((InterruptType_Type *) SCS_BASE)         /*!< Interrupt Type Register           */
//#define SCB                 ((SCB_Type *)           SCB_BASE)         /*!< SCB configuration struct          */
//Systick moved to stm_systick.h #define SysTick             ((SysTick_Type *)       SysTick_BASE)     /*!< SysTick configuration struct      */
...
#if 0 //AK: Those are intrinsic functions!
...
{
  uint32_t reg_value;
  uint32_t PriorityGroupTmp = (PriorityGroup & 0x07U);                         /* only values 0..7 are used          */
...
{
  NVIC->ISER[((uint32_t)(IRQn) >> 5)] = (1U << ((uint32_t)(IRQn) & 0x1FU)); /* enable interrupt */
}
...
{
  NVIC->ICER[((uint32_t)(IRQn) >> 5)] = (1U << ((uint32_t)(IRQn) & 0x1FU)); /* disable interrupt */
}
...
{
  return((uint32_t) ((NVIC->ISPR[(uint32_t)(IRQn) >> 5] & (1U << ((uint32_t)(IRQn) & 0x1FU)))?1:0)); /* Return 1 if pending else 0 */
}
...
{
  NVIC->ISPR[((uint32_t)(IRQn) >> 5)] = (1U << ((uint32_t)(IRQn) & 0x1FU)); /* set interrupt pending */
}
...
{
  NVIC->ICPR[((uint32_t)(IRQn) >> 5)] = (1U << ((uint32_t)(IRQn) & 0x1FU)); /* Clear pending interrupt */
}
...
{
  return((uint32_t)((NVIC->IABR[(uint32_t)(IRQn) >> 5] & (1U << ((uint32_t)(IRQn) & 0x1FU)))?1:0)); /* Return 1 if active else 0 */
}
...
static __INLINE void NVIC_SetPriority(IRQn_Type IRQn, uint32_t priority)
{
    if((int32_t)IRQn < 0)
    {
        SCB->SHP[((uint32_t)(IRQn) & 0xFU)-4] = ((priority << (8 - __NVIC_PRIO_BITS)) & 0xffU);
    } /* set Priority for Cortex-M3 System Interrupts */
    else
    {
        NVIC->IP[(uint32_t)(IRQn)] = ((priority << (8 - __NVIC_PRIO_BITS)) & 0xffU);
...
static __INLINE uint32_t NVIC_GetPriority(IRQn_Type IRQn)
{
    ...
    if((int32_t)IRQn < 0)
    {
        ret = ((uint32_t)(SCB->SHP[((uint32_t)(IRQn) & 0xFU)-4] >> (8 - __NVIC_PRIO_BITS)));
    } /* get priority for Cortex-M3 system interrupts */
    else
    {
        ret = ((uint32_t)(NVIC->IP[(uint32_t)(IRQn)]           >> (8 - __NVIC_PRIO_BITS)));
...
#if 0 ////Systick moved to stm_systick.h
...
static __INLINE uint32_t SysTick_Config(uint32_t ticks)
{
  ...
  NVIC_SetPriority (SysTick_IRQn, (1U<<__NVIC_PRIO_BITS) - 1);  /* set Priority for Cortex-M0 System Interrupts */
