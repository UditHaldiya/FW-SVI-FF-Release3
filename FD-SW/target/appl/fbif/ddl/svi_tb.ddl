...
METHOD block_mode_auto
{
	...
	DEFINITION{
		...
		id = ITEM_ID(PARAM.MODE_BLK);
		mb = MEMBER_ID(TARGET);
		status = select_from_menu("|en|\n Warning: Please note that different coding of the modes will be used between the FF and APP processors.\n" \
                                  " Do you wish to continue?\n",
                                  dummy, dummy, 0, "Yes;No(skip)", &ivalue);
		if(ivalue == 1) {
			status = put_string_value(id, mb,"|en|Manual",10);
			display_message("|en|Going into AUTO mode", dummy, dummy, 0);
			}

	}
}
METHOD block_mode_manual
{
	LABEL "|en|MAN";
	 CLASS OUTPUT;
	DEFINITION{
	...
	mbs[0] = MEMBER_ID(TARGET);
	status = select_from_menu("|en|\n Warning: Placing the transducer block in Manual will hold the value at the current point (FINAL_POSITION_VALUE)\n" \
                                  " Do you wish to continue?\n",
                                  dummy, dummy, 0, "Yes;No(skip)", &ivalue);
	if(ivalue == 1) {
			/*mbs[0] = 16;*/
			status = edit_device_value("|en|\n MODE_BLK.TARGET:\n",
                                       ids, mbs, 1, ids[0], mbs[0]);
			/*display_message("|en|Going into MAN mode", dummy, dummy, 0);*/

		}
	}
}
METHOD block_mode_outOfService
{
	LABEL "|en|OOS";
	 CLASS INPUT;
	DEFINITION{
	...
	ids[0] = ITEM_ID(PARAM.MODE_BLK);
	mbs[0] = MEMBER_ID(TARGET);
	status = select_from_menu("|en|\n Warning: Switching the block to OOS mode will stop block execution and will force the valve to non-powered condition.\n" \
                                  " Do you wish to continue?\n",
                                  dummy, dummy, 0, "Yes;No(skip)", &ivalue);
	if(ivalue == 1) {
			/*display_message("|en|Going into OOS mode \n", dummy, dummy, 0);
			status = put_unsigned_value(ids[0], mbs[0], 128);*/
			display_message("|en|\nTarget %[d]{2}:\n\n", ids, mbs, 0);
			delayfor(3,  "|en|\nTarget %[d]{2}:\n\n",    \
                                 ids, mbs, 0);
		}
...
{
    LABEL "|en|Start PST";
	HELP "|en|Check if process is running and whether percent complete is > 150%";
    CLASS INPUT;
    DEFINITION
    {
		COMMON_LOCAL_VARS;
		SEND_COMMAND(90);
		/*if (mbs[1] >= 150) {
			get_acknowledgement("|en|Proceeding will risk losing data in buffer. \n Clixk Next to proceed or quit now to keep data. \n", dummy, dummy, 0);
		}
		else if (mbs[1] < 150){
			SEND_PARAM(ids[2], mbs[2], 90);
			status = put_unsigned_value(ids[2], mbs[2], 90);
			status = send_value(ids[2],mbs[2]);
			send_all_values();
			delayfor(2, "|en|PST is running... \n", dummy, dummy, 0);
			abortme = 0;
		}
		while (abortme != 0){
			delayfor(2, "|en|Running PST ... ", dummy, dummy, 0);

			SEND_PARAM(ids[2], mbs[2], 99);
			status = put_unsigned_value(ids[2], mbs[2], 99);
			status = send_value(ids[2],mbs[2]);
			send_all_values();
...
METHOD  do_manual_hi_low_stops
{
    ...
    DEFINITION
    {
        ...
        unsigned long func;
        unsigned long ulCal_Type;
		unsigned long raw_pos_lo;
		unsigned long raw_pos_hi;
		int choice;
        ...
        status = select_from_menu("|en|\n Select Manual Stop(s) to set:\n", \
                                  ids, mbs, 9, \
                                  "HI/LO Limits;HI Limit;LO Limit;Abort", &ivalue);
		...
		switch (sel){
		case FINDSTOPS_MANUL_LO_HI:
			sel = FINDSTOPS_SETOPEN;
		case FINDSTOPS_SETOPEN:
		{

			status = get_unsigned_value(ids[6], 0, &raw_pos_hi);
			status = get_status_string(ids[3], mbs[3], FINDSTOPS_SETOPEN, str_buf, maxlen);

			SEND_PARAM(ids[3], 0, "FINDSTOPS_SETOPEN");

			if (SVI2_DBL_ACT == act_style ){

			display_dynamics(	"|en| Wait for Raw Position to stabilize. Click Next to commit to the final value. Click Cancel otherwise. \n " \
								"|en|\nRun %{str_buf}:\n\n" \
                                 "  ACTUATOR A PRESSURE = %[7.2f]{4} %{strPress_Unit}\n" \
                                 "  ACTUATOR B PRESSURE = %[7.2f]{5} %{strPress_Unit}\n" \
                                 "  SUPPLY PRESSURE = %[7.2f]{8} %{strPress_Unit}\n"        \
			...
			status = select_from_menu("|en|\nRun %{str_buf}:\n\n" \
                                 ...
                                 "  RAW POSITION = %[8i]{6}\n"          \
                                 "  ACTUAL POSITION = %[7.2f]{7}\n",    \
                                      ids, mbs, 9, "Accept current stop;Cancel to abort operation", &ivalue);

				if (ivalue == 1 && ulCal_Type == CAL_TYPE_BOTH_ENDPOINTS_MANUAL){
					 sel = FINDSTOPS_SETCLOSED;
					 func = FINDSTOPS_SETCLOSED;
					 status = get_unsigned_value(ids[6], 0, &raw_pos_hi);
					 SEND_PARAM(ids[3], 0, "FINDSTOPS_SETCLOSED");
				}else if(ivalue == 1){
					 SEND_PARAM(ids[6], 0, raw_pos_hi);
					 status = get_unsigned_value(ids[6], 0, &raw_pos_hi);
					 break;
				}
				else if (ivalue == 2){
		    ...
		    /*status = select_from_menu("|en|\nRun %{str_buf}:\n\n" \
				...
				if (ivalue == 1 && ulCal_Type == CAL_TYPE_BOTH_ENDPOINTS_MANUAL){
					 ...
					 status = put_unsigned_value(ids[6], 0, raw_pos_hi);
					 break;
				}
				else if (ivalue == 2){
					METHOD_ABORT_MACRO;
	...
	if(choice == 1){
		...
		{
		status = get_unsigned_value(ids[6], 0, &raw_pos_lo);
		...
		if(choice == 1){
			status = put_unsigned_value(ids[6], 0, raw_pos_lo);
			SEND_PARAM(ids[6], 0, raw_pos_lo);
			display_message("|en|\n RAW POSITION LO = %[8i]{6}\n",  ids, mbs, 9);
			status = put_unsigned_value(ids[6], 0, raw_pos_hi);
			SEND_PARAM(ids[6], 0, raw_pos_hi);
			display_message("|en|\n RAW POSITION HI = %[8i]{6}\n",  ids, mbs, 9);
		}else if (choice == 2){
			status = put_unsigned_value(ids[6], 0, raw_pos_hi);
			SEND_PARAM(ids[6], 0, raw_pos_hi);
			display_message("|en|\n RAW POSITION HI = %[8i]{6}\n",  ids, mbs, 9);
	...
	send_all_values();
        ...
        display_message("|en|\nThe Manual Stop Limit finished.", dummy, dummy, 0);
